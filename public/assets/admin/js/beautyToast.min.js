class BeautyToast {
    constructor() {
        this.darkTheme = !1, this.iconWidth = null, this.iconHeight = null, this.iconSize = null, this.closeButton = !0, this.timeout = 2500, this.animationTime = 150
    }
    ...
    success({
                ...
                messageColor: i = null,
                titleColor: o = null,
                progressBarColor: n = null,
                topbarColor: a = null,
                iconColor: r = null,
                darkTheme: u = this.darkTheme,
                iconWidth: h = this.iconWidth,
                iconHeight: c = this.iconHeight,
                iconSize: C = this.iconSize,
                closeButton: d = this.closeButton,
                timeout: p = this.timeout,
                animationTime: y = this.animationTime,
                onClose: T = null,
                onOpen: g = null
            } = {}) {
        this.checkAndAddToastWrapper.then(m => {
            let v = m.children.length + 1,
                w = null == l ? `<svg width="${h ?? "32"}" height="${c ?? "32"}" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M27.2309 9.65179L11.731 25.1518L4.62679 18.0476L6.44803 16.2264L11.731 21.4964L25.4097 7.83054L27.2309 9.65179Z" fill="${r ?? "#68D84C"}"/></svg>` : `<i class="${l}" style="font-size: ${null != h ? C + "px;" : "32px;"}"></i>`,
                b = this.toastGenerator({
                    toastType: "success",
                    darkTheme: u,
                    closeButton: d,
                    toastId: v,
    ...
    checkAndAddToastWrapper = new Promise((_, t) => {
        let e = document.querySelector(".beautyToast-wrapper");
        if (null == e) {
            let l = document.querySelector("body"), s = document.createElement("div");
            s.classList.add("beautyToast-wrapper"), l.appendChild(s), _(s)
        } else t("Can't create BeautyToast Wrapper!")
    });
    processToast = ({toast: _, toastId: t, zIndex: e, timeout: l, animationTime: s, onClose: $, onOpen: i}) => {
        new Promise((l, s) => {
            let $ = document.querySelector(".beautyToast-wrapper");
            $.setAttribute("style", `z-index: ${e} !important;`), $.innerHTML += _, l(t)
        }).then(async _ => {
            let t = document.querySelector(`.beautyToast[toast-id="${_}"]`);
            return setTimeout(() => {
                t.classList.add("show")
            }, 50), setTimeout(() => {
                t.classList.add("animate-child")
            }, s), null != i && i(), _
        }).then(async _ => {
            let t = l, e = document.querySelector(`.beautyToast[toast-id="${_}"]`), s = setInterval(() => {
                let i = Math.floor((t -= 100) / l * 100);
                e.querySelector(".beautyToast-progress").style.width = i + "%", 0 == t && (clearInterval(s), closeToast(_, $))
            }, 100)
        })
    };

    destroy() {
        let _ = document.querySelector(".beautyToast-wrapper");
        this.fadeOut(_), setTimeout(() => {
            _.remove()
        }, 300)
    }

    settings({darkTheme: _, iconWidth: t, iconHeight: e, iconSize: l, closeButton: s, timeout: $, animationTime: i}) {
        void 0 != _ && (this.darkTheme = _), void 0 != t && (this.iconWidth = t), void 0 != e && (this.iconHeight = e), void 0 != l && (this.iconSize = l), void 0 != s && (this.closeButton = s), void 0 != $ && (this.timeout = $), void 0 != i && (this.animationTime = i)
    }
    ...
    toastGenerator({
        ...
        let C = `<div class="beautyToast ${t ? "beautyToast-dark" : ""} beautyToast-${_}  ${e ? "" : "no-close"}" toast-id="${l}" ${null != s ? 'id="' + s + '"' : ""} style="${null != $ ? "background-color: " + $ + "!important;" : ""}"><div class="beautyToast-topbar" ${null != i ? 'style="background-color:' + i + '!important;"' : ""}></div><div class="beautyToast-body"><div class="beautyToast-title-group"><div class="beautyToast-icon">${o}</div><h5 class="beautyToast-title" style="${null != n ? "color: " + n + "!important;" : ""}">${a}</h5></div><p class="beautyToast-text" style="${null != r ? "color: " + r + "!important;" : ""}">${u}</p></div>${e ? `<button class="beautyToast-close" onclick="closeToast(${l}, ${c})">&times;</button>` : ""}<div class="beautyToast-progress" style="${null != h ? "background-color:" + h + "!important;" : ""}"></div></div>`;
        return C
    }

    fadeOut(_) {
        var t = setInterval(function () {
            _.style.opacity || (_.style.opacity = 1, _.style.transition = "all .2s linear"), _.style.opacity > 0 ? _.style.opacity -= .1 : clearInterval(t)
        }, 20)
    }
}

const closeToast = (_, t) => {
    new Promise((t, e) => {
        let l = document.querySelector(`.beautyToast[toast-id="${_}"]`);
        null != l && (l.classList.remove("show"), t(l))
    }).then(_ => (setTimeout(async () => {
        _.style.display = "none", _.remove()
    }, 400), !0)).then(_ => {
        null != t && t()
    })
};
